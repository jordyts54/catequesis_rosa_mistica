  ...
  template: `
    <div class="form-container">
      <div class="form-header">
        ...
        ></button>
        <h1>Editar Asistencia Individual</h1>
        <div></div>
      </div>
      ...
      <form [formGroup]="editForm" (ngSubmit)="onSubmit()" class="form-content" *ngIf="editForm && attendance">
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input
              pInputText
              [value]="attendance.encounter?.fecha || 'N/A'"
              disabled
              class="w-full"
            />
          </div>
...
export class AttendancesEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Cargar opciones primero, luego el registro
  ...
  loadEstadoOptions(): void {
    this.parameterTypesService.getByType('AsistenciaEncuentro').subscribe({
      next: (items) => {
        ...
        // Cargar el registro DESPUÉS de tener las opciones
  ...
  loadAttendance(): void {
    ...
    this.attendancesService.getById(this.attendanceId).subscribe({
  ...
  getStudentName(): string {
    if (this.attendance?.student?.person) {
      return `${this.attendance.student.person.nombres} ${this.attendance.student.person.apellidos}`;
    }
    return 'N/A';
  }

  getStudentCedula(): string {
    return this.attendance?.student?.person?.cedula || 'N/A';
  }

  getEncounterInfo(): string {
    if (this.attendance?.encounter) {
      return this.attendance.encounter.tema || 'N/A';
    }
    return 'N/A';
  }
  ...
  onSubmit(): void {
    ...
    this.attendancesService.update(this.attendanceId, data).subscribe({
      next: () => {
        this.isSaving = false;
